...
import com.example.FixLog.domain.post.dto.PostDto;
...
import com.example.FixLog.domain.member.MemberService;
import com.example.FixLog.infra.s3.S3Service;
...
public class PostService {
    ...
    private List<Tag> fetchAndValidateTags(List<String> tagNames){
        ...
        if (!issues.isEmpty()) {
            ...
            throw new CustomException(ErrorStatus.REQUIRED_TAGS_MISSING, message);
        }
        return tags;
    }
    ...
    private void validatePost(PostRequestDto postRequestDto){
        if (!StringUtils.hasText(postRequestDto.getPostTitle())
            ...
            throw new CustomException(ErrorStatus.REQUIRED_CONTENT_MISSING);
    }
    ...
    public void editPost(Long postId, PostRequestDto postRequestDto) {
        ...
        if (!member.getUserId().equals(post.getUserId().getUserId())) {
            throw new CustomException(ErrorStatus.POST_UPDATE_FORBIDDEN);
        }
        ...
        if (Objects.equals(post.getPostTitle(), postRequestDto.getPostTitle())
            ...
            throw new CustomException(ErrorStatus.NO_CONTENT_CHANGED);
        }
    ...
    public String toggleBookmark(Long postIdInput){
        ...
        if (member == postId.getUserId())
            throw new CustomException(ErrorStatus.SELF_BOOKMARK_NOT_ALLOWED);
        // 북마크 처리
